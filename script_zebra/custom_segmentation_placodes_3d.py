...
parser.add_argument('-xyd', type=int, help='xy diameter for Cellpose Segmentation', default=50)
parser.add_argument('-zd', type=int, help='xz and yz diameter for Cellpose Segmentation', default=30)
...
model = CellposeModel(pretrained_model=model_type, gpu=use_gpu)
...
for i, filename in enumerate(images_path):
    ...
    xy_masks, _, _ = model.eval(list(img), flow_threshold=flow_threshold, channels=[0, 0], diameter=xy_diameter)
